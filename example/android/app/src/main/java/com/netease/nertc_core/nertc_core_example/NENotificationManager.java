...
class NENotificationManager {
  ...
  public void createForegroundNotificationChannel(Context context) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      ForegroundServiceConfig config = CallForegroundService.getForegroundServiceConfig();
      if (config == null) {
        return;
      }
  ...
  public Notification buildForegroundNotification(Context context) {
    ...
    Notification notification =
        new NotificationCompat.Builder(context, config.channelId)
            .setContentTitle(config.contentTitle)
            .setContentText(config.contentText)
            .setSmallIcon(
                config.smallIcon == 0 ? context.getApplicationInfo().icon : config.smallIcon)
